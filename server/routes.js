            ...
            youtube.videos.list({
                ...
                if (err) {
                    res.end(JSON.stringify({
                        error: true,
                        explanation: "can't find the youtube video provided"
                    }));
                    return;
                }
                if (data.items === undefined || data.items === null || data.items[0] === undefined || data.items[0] === null) {
                    res.end(JSON.stringify({
                        error: true,
                        explanation: "unknow Youtube video, please double check the URL provided"
                    }));
                    return;
                }
                ...
                rapgeniusClient.searchSong(normalizedYoutubeTitle, "rap", function(err, songs) {
                    if (err) {
                        res.end(JSON.stringify({
                            error: true,
                            explanation: "problem with the Genius.com API"
                        }));
                        return;
                    } else {
                        if (songs === undefined || songs[0] === undefined || songs[0] === null) {
                            res.end(JSON.stringify({
                                error: true,
                                explanation: "can't find any lyrics for the youtube link provided"
                            }));
                            return;
                        }
                        ...
                        rapgeniusClient.searchLyricsAndExplanations(songs[iMostSuitable].link, "rap", function(err, lyricsAndExplanations) {
                            if (err) {
                                res.end(JSON.stringify({
                                    error: true,
                                    explanation: "can't find the lyrics for the youtube link provided"
                                }));
                                return;
                            } else {
                                res.end(JSON.stringify({
